...
def addBoard(item):
    ...
    context = {'indent': INDENT_STR, 'playall_title': playall_title, 'playall_icon': iconimage}
    addDir(name, item_url, mode, iconimage, context=context)


# 歌手
def addArtistMusicNav(item):
    addNav(item, 'processWebPage(artistMusicPage)')


def addArtistNav(item):
    addNav(item, 'processWebPage(artistListPage)')
...
def addArtistList(item):
    ...
    addDir(name, item_url, 'processWebPage(artistMusicPage)', iconimage)
...
def addArtistCategory(item):
    ...
    addDir(name, item_url, 'processWebPage(artistListPage)', '')
...
def addHotMusicList(item):
    ...
    playall_title = '播放【%s】所含曲目' % name
    context = {'indent': INDENT_STR, 'playall_title': playall_title, 'playall_icon': iconimg}
...
def addAlbum(item):
    ...
    dispname = title
    context = {'indent': INDENT_STR, 'playall_title': playall_title, 'playall_icon': iconimg}
    dispname = params.get('indent', '') + dispname
...
def addPlaylist(item):
    ...
    for item in items:
        ...
        item_url = extractHref(item)
        iconimage = extractImgSearch(item, name)
        addDir(name, item_url, 'processWebPage(playlistAlbum)', iconimage)
