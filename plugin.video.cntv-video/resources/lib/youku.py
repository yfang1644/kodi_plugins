...
class Youku():
    ...
    def prepare(self, **kwargs):
        ...
        if self.url and not self.vid:
            self.vid = self.vid_from_url(self.url)
        ...
        try:
            meta = simplejson.loads(get_content(
                api_url,
                headers={'Referer': 'http://static.youku.com/'}
            ))
            meta12 = simplejson.loads(get_content(
                api12_url,
                headers={'Referer': 'http://static.youku.com/'}
            ))
            data = meta['data']
            ...
            try:
                ...
                for piece in pieces:
                    ...
                    for no in range(0, seg_count):
                        ...
                        ksegs += [i['server'] for i in simplejson.loads(get_content(u))]
