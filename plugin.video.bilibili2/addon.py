...
def _play_video_by_list(urls_info, show_comments='0'):
    bili._print_info('Play without subtitle')
    playlist = xbmc.PlayList(1)
    playlist.clear()
    i = 1
    bili._print_info(str(len(urls_info[0])) + ' parts found!')
    player = BiliPlayer()
    for url in urls_info[0]:
        ...
        i += 1
        url += '|Referer=http://www.bilibili.com'
        playlist.add(url, listitem=list_item)
    player.showSubtitles(False)
    player.show_subtitle = False
    player.play(playlist)
    #if show_comments == '1':
        #xbmc.Player().setSubtitles(bili._get_tmp_dir() + '/' + urls_info[1])
...
def _play_video(urls_info, show_comments='1'):
    ...
    urls = []
    for url in urls_info[0]:
        urls.append(url + '|Referer=http://www.bilibili.com')
    ...
    player = BiliPlayer()
    if show_comments == '1':
        bili._print_info('Play with subtitle')
        player.setSubtitle(bili._get_tmp_dir() + '/' + urls_info[1])
    else:
        bili._print_info('Play without subtitle')
        player.showSubtitles(False)
        player.show_subtitle = False
...
def searchvideo():
    ...
    search video
    """
    kb = xbmc.Keyboard('',u'请输入AV号(不包括AV两字)')
    kb.doModal()
...
def play_video(url, by_list, show_comments):
    if show_comments == '1':
        bili._print_info('Fetch subtitle')
        playlist = bili.get_video_urls(url, True)
    else:
        bili._print_info('Don\'t fetch subtitle')
        playlist = bili.get_video_urls(url, False)
    n = len(playlist[0])
    bili._print_info('%d videos found' % n)
    if n > 0:
        if by_list == '1':
