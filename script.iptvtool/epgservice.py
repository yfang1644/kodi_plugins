...
def updateChannelCCTV(fHandle, channelID, channelName):
    try:
        ...
        #Get data
        data = get_html("http://api.cntv.cn/epg/epginfo?serviceId=tvcctv&c=%s&d=%s" % (channelID, dateInChina.strftime("%Y%m%d")))
        programmes = loads(data)[channelID]['program']
...
def updateChannelPHNX(fHandle, channelID, siteID, channelName):
    try:
        ...
        #Get data
        data = get_html("https://www.tvsou.com/epg/%s" % (siteID))
