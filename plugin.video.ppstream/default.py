...
def progListMovie(name, id, page, cat, area, year, order, listpage):
    ...
    if listpage is None:  # first entry
        p_url = PPS_URL + '/v_list/'+id+'_p_1.html'
    ...
    url = PPS_URL + '/v_list/'+videoID+catID+areaID+yearID+sortID+'_p_'+page
    url += '.html'
    ...
    if re.search(cat,'全部'):
        cat = '全部类型'
    if re.search(area,'全部'):
        area = '全部地区'
    if re.search(year,'全部'):
        year = '全部年份'
...
def progListSeries(name, url, thumb, episodeSel):
    ...
    s_url = PPS_URL + '/ugc/ajax/aj_newlongvideo.php?&from=web&type=play&sid=' + sid
    ...
    if jsd['have_time'] == "yes":
        for tdate in sorted(vlist.iterkeys(), reverse=True):
            ...
            for i in range(0, len(alist)):
                ...
                p_url = PPS_URL + '/play_' + v_url + '.html#from_splay'
                #p_list += ' [' + p_time + '] '
...
def progListUgc(name, id, page, cat, year, order):
    ...
    if cat[0] == 'c':
        ...
        url = PPS_URL + '/'+id+'/list-'+cat+'-'+dateID+'-'+orderID+'-p'+page+'.html'
    else:
        mf ='13'
    ...
    if len(matchp):
        ...
        if len(match):
            ...
            for i in range(0, len(match)):
                ...
                if match1:
                    p_list = '[COLOR FF00FF00]'+match1[0]+'[/COLOR] ' + p_name

                match1 = re.compile('<p class="viedo_rb">.+?<span class="v_name">([:0-9]+)</span>').findall(match[i])
                if match1:
                    p_list += ' [COLOR FFFF00FF]['+match1[0]+'][/COLOR] '
...
def ppsSearchListx(name, url, page):
    ...
    if len(match_ml):
        for x in range(0, len(match_ml)):
            ...
            if match1 is None:
                continue   # Proceed only if provider sites found
...
#                        mode = fetchID(MPLAYER_LIST,match1[j][1])
                        mode = fetchID(MPLAYER_LIST, match1[0])
                        if mode =='':
                            mode = '99' # player not implemented
